...
* **Coverage**
  Average number of bits read per synthesized bit:
...
## Example
We can define the parameters of a system as found once all data has been decoded successfully.
...
<!-- TODO (Atar) How to merge with previous slide (remove/merge/copy) -->
...
<!-- TODO: (Atar): Visualize the notations -->
...
<!-- TODO(Atar): Can mention (should not go too deep!) about the multinomial approximation -->
...
- BEC is a channel where each transmitted bit is erased with probability $\varepsilon$.

- Input: $n$ bits from $\{0, 1\}$.
- Output: $n$ bits from $\{0, 1,?\}$, where $?$ denotes an erasure.

Each bit is either received correctly or erased with probability $\varepsilon$.
...
- The entropy is always between 0 and 1: $$0 \leq H(X) \leq 1$$
...
- The difference between BSC and BEC is that in BSC, the output is always a bit (0 or 1), while in BEC, the output can also be an erasure symbol (?). (We do not know where the erasure occurred, but we know that it did occur).

<!-- TODO(ATAR): Add an image -->
...
* The authors did not elaborate on how they ferived the lower bound on $c_r$ using this probability, it was achieved by numerical calculations.

<!-- TODO(Atar): mention that it is no tight bound -->
...
<!-- TODO(Atar): Visualize the strategy -->
...
<!-- TODO(Atar) encode + bit flip decode -->
